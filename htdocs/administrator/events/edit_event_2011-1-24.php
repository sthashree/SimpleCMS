	...
	<table><tr><td style="padding-left:8px"><textarea name="data[event][short_description][<?=$totalLanguage->abbr;?>]" cols="55" id="short_description_<?=$totalLanguage->id;?>" tabindex="7" title="short_description"><?=getLanguageSpecific($event->short_description,$totalLanguage->abbr);?></textarea>
		...
		<?php } ?>
			...
			<label for="meta" class="required"><?php echo $t->translate('event/start-date'); ?></label>
				<input type="text" id="starttime" name="data[start_date][day]" tabindex="1" size="9" value="<?=$start_time['day'];?>" title="start-time">
                <?=$objEvent->getHours('start_date',$start_time['hour']);?>&nbsp;<?php echo $t->translate('event/h'); ?>&nbsp;&nbsp;<?=$objEvent->getMinutes('start_date',$start_time['minute']).$t->translate('event/m'); ?><br/>
			<label for="meta" class="required"><?php echo $t->translate('event/end-date'); ?></label>
				<input type="text" id="endtime" name="data[end_date][day]" tabindex="1" size="9" value="<?=$end_time['day'];?>" title="end-time">
                <?=$objEvent->getHours('end_date',$end_time['hour']);?>&nbsp;<?php echo $t->translate('event/h'); ?>&nbsp;&nbsp;<?=$objEvent->getMinutes('end_date',$end_time['minute']).$t->translate('event/m'); ?><br/>
