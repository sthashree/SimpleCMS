<?php 
	...
	$category = $productObj->Find(0,null,$_GET['category']);
?>

<h2> <?php echo $t->translate('product/addcategory');?> </h2>	
	 <?php $category_full = $productObj->Find(); ?>
                ...
                <input type="hidden" name="id" size="50" value="<?=$category->id;?>" title="">
<fieldset>
	<?php if($category->parent_id!=0) { ?>
	<label for="title" class="required"><?php echo $t->translate('myproduct/category-parent'); ?></label>
	<?php echo SelectList("data[Category][parent_id]",$category_full,"category_name",$category->parent_id); ?><br />
    <?php } ?>
	<label for="title" class="required"><?php echo $t->translate('myproduct/category-title'); ?></label>
		<input type="text" id="title" name="data[Category][category_name]" tabindex="2" size="50" value="<?=$category->category_name;?>" title="filesize"><br/>

	<label for="description" class="required"><?php echo $t->translate('desc'); ?></label>	
	<table><tr><td style="padding-left:8px"><textarea name="data[Category][category_description]" cols="55" id="category_description" tabindex="3" title="gallery_description"><?=$category->category_description;?></textarea>
</td></tr></table>	
