...
?>
    ...
    <label for="Date" class="required"><?php echo $t->translate('date'); ?></label>
               <input type="text" name="data[<?=$i;?>][chirasidate]" onClick="cal_<?=$i;?>.write();" onChange="cal_<?=$i;?>.getFormValue(); cal_<?=$i;?>.hide();">
               <div id="calid_<?=$i;?>"></div>
        </div>
        <div>
        <label for="published" class="required"><?php echo $t->translate('front'); ?></label>
                <select name="data[<?=$i;?>][front]" id="published" tabindex="10">
                    <option value="1" selected="selected"><?php echo $t->translate("front"); ?></option>
                    <option value="0"><?php echo $t->translate("back"); ?></option>
                </select>
             </div>
             <div>   
...
<input type="hidden" name="action" value="finalchirasi"/><!-- -->
