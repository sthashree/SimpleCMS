...
class Blog extends DBObject  {
	...
	function Find( $category=0, $limit = null, $id = 0, $field='id',$order='desc',$language = 0,$where)
	{
		global $CONN_STRING,$USER_NAME,$PWD;
		try{
					$conn = new PDO($CONN_STRING,$USER_NAME,$PWD);
					...
					if(intval($id) > 0)
					{
						$sql = "select t.*, u.username from ".$this->_uses." t inner join stha_users u on t.user_id = u.id where t.id = ".$id."  order by $field $order limit 0,1";
						$result = $conn->query(trim($sql));
					    $result->setFetchMode(PDO::FETCH_CLASS,Stdclass);
						$item = $result->fetch();
					}
	...
	function SearchTitle($search_string,$limit=null)
	{
		...
		try{
					...
					$conn->query("SET CHARACTER SET utf8");
					$conn->query("SET SESSION collation_connection='utf8_general_ci'");
					$sql = "select t.*,dATE_FORMAT(t.updated_date,'%Y-%m-%d') as modified_date, u.username from ".$this->tblTitle." t inner join stha_users u on t.user_id = u.id where $search_string order by id desc $limit";			  			
					$result = $conn->query(trim($sql));
				    $result->setFetchMode(PDO::FETCH_CLASS,Stdclass);
					while($data = $result->fetch())
					{
	...
	function FindTitle( $category="", $limit = null, $id = 0, $field='id',$order='desc',$language = 0,$where)
	{
		global $CONN_STRING,$USER_NAME,$PWD;
		try{
					$conn = new PDO($CONN_STRING,$USER_NAME,$PWD);
	...
	function SaveComment($id = 0)
	{
		...
		if(is_array($this->_data))
		{
			...
			try{
					...
					{
						//echo "insert into ".$this->tblComment."(".join(',',$elements).") values(".what(count($elements)).")";
	...
	{
		$dates = explode(" ",$date);
		return explode("-",$dates[0]);	
	}
	
	function makeLink($get,$year,$month)
	{
		$links = ($get);
		
		$finallink=$month."/".$year."/";
		foreach($links as $key => $link) {
			...
			if($final["name"]!="") { $finallink.=$final["name"]."/".$final['extra'].".htm"; }
			else { $finallink.=$final["extra"].".htm"; }
	...
	{
		$publicCategories = $this->Search("t.public = '1'");
	//	var_dump($publicCategories);
